	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		} else if (emailExistente == 1)
		{
			...
			if (senhaExistente.equals(uSenha))
			{
				...
				retornaRequest(request, response, "Entrando no sistema!", usuario, "login.jsp");

			} else if (senhaExistente != uSenha)
			{
				retornaRequest(request, response, "Senha errada. ", usuario, "login.jsp");
			} else
			{
				retornaRequest(request, response, "Ocorreu um erro, verifique email e senha.", usuario, "login.jsp");
			}
		}
